/* eslint-disable jsx-a11y/alt-text */
/* eslint-disable @next/next/no-img-element */
"use client"
import Navbar from "../../components/navbar";
import Footer from "../../components/footer";
import { useEffect } from "react";

export default function Home() {
  useEffect(() => {
    import("preline");
  }, []);
  return (
    <>
    <Navbar/>
  <main className=" text-dark py-28 bg-cover w-full ">
    <img
      src="data:image/png;base64,"
      alt=""
      className="absolute"
    />
    <div className="flex flex-col gap-4 justify-center items-center">
      <h1 className="font-bold md:text-4xl text-2xl text-center ">
        Front-End Web Developer
      </h1>
      <p className="text-center font-medium md:text-xl md:w-[50%] text-sm px-5">
        Kurikulum disusun oleh lopercourse dan pelaku industri di bidang Web
        Development. Siswa dipersiapkan untuk menjadi Front-End Web Developer
        sesuai standar kebutuhan industri.
      </p>
    </div>
  </main>
  <main className="mt-32 px-3 md:px-20 w-full">
    <div className="flex-col md:flex  justify-between w-full items-center">
      <div className=" px-6 md:w-[370px] sm:h-[164px] flex items-center hover:shadow-lg mx-auto h-auto py-2 border rounded-md border-gray-600 ">
        <div className="gap-2 md:gap-3 flex flex-col">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 16 16"
              width={16}
              height={16}
              className="mr-2"
            >
              <path fill="#fff" d="M0 0h16v16H0z" />
              <path
                stroke="#3F3F46"
                strokeLinejoin="round"
                strokeWidth="1.5"
                d="M13.5 13.5h-11V9h3V5.5h4V2h4v11.5z"
              />
            </svg>
            <h1 className="font-bold text-xs">Langkah 1</h1>
          </div>
          <span className="font-bold text-base">
            Belajar Dasar Pemrograman Web
          </span>
          <div className="flex gap-9">
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="text-green-400"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M8 14.667A6.667 6.667 0 1 0 8 1.334a6.667 6.667 0 0 0 0 13.333Zm.667-10a.667.667 0 0 0-1.333 0v3.334c0 .176.07.346.195.471l2.334 2.333a.667.667 0 0 0 .942-.942L8.667 7.725V4.667Z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-semibold text-xs">41 jam</p>
            </div>
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
                className="course-card__icon text-yellow-400"
              >
                <path
                  d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                  fill=""
                />
              </svg>
              <p className="font-semibold text-xs">4,87</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-400"
                width={16}
                height={16}
              >
                <rect
                  width="2.667"
                  height="4.667"
                  x="2.667"
                  y={8}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="6.667"
                  x="6.667"
                  y={6}
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="9.333"
                  x="10.667"
                  y="3.333"
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
              </svg>
              <p className="font-semibold text-xs">Dasar</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                className="course-card__icon text-gray-500"
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1.33334 2.66666C1.33334 1.93028 1.9303 1.33333 2.66668 1.33333H10C10.7364 1.33333 11.3333 1.93028 11.3333 2.66666V3.33333H13.3333C14.0697 3.33333 14.6667 3.93028 14.6667 4.66666V13.3333C14.6667 14.0697 14.0697 14.6667 13.3333 14.6667H2.66668C1.9303 14.6667 1.33334 14.0697 1.33334 13.3333V2.66666ZM10 2.66666V3.33333H2.66668V2.66666H10ZM2.66668 13.3333V4.66666H4.66668V13.3333H2.66668ZM6.00001 13.3333H13.3333V4.66666H6.00001V13.3333Z"
                ></path>
              </svg>
              <p className="font-normal text-xs">115,modul</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-500"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M5 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM4 5a1 1 0 112 0 1 1 0 01-2 0zM11 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM10 5a1 1 0 112 0 1 1 0 01-2 0zM8 9.558a3.667 3.667 0 00-6.667 2.109V14c0 .368.299.667.667.667h12a.667.667 0 00.667-.667v-2.333A3.667 3.667 0 008 9.558zm-.667 3.775v-1.668a2.333 2.333 0 00-4.666.002v1.666h4.666zm1.334-1.668v1.668h4.666v-1.666a2.333 2.333 0 00-4.666-.002z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-normal text-xs">125.400 Siswa terdaftar</p>
            </div>
          </div>
        </div>
      </div>
      <div className="flex mx-auto md:gap-10 gap-1 flex-col">
        <div className="h-10 md:h-32 bg-dark w-[2px] md:mt-0 mt-2  mx-auto flex" />
        <span className="font-bold flex justify-center ">1</span>
        {/* <span class="font-bold items-center text-orange-500 hidden md:flex">2</span> */}
      </div>
      <span className="flex text-center  md:mt-0 mt-10 flex-col gap-4">
        <h1 className="font-bold text-xl">Buat Web Pertamamu di Sini</h1>
        <div className="flex justify-center">
          <p className="font-medium text-center md:w-[60%]">
            Belajar dasar adalah langkah yang tidak boleh dilewatkan untuk mahir
            di bidang apa pun. Di sini, Anda akan belajar dasar dari HTML dan
            CSS sebagai pilar pembentuk website.
          </p>
        </div>
      </span>
    </div>
    <div className="flex-col md:flex mt-8 justify-between w-full items-center">
      <div className=" px-6 md:w-[370px] sm:h-[164px] flex items-center hover:shadow-lg mx-auto h-auto py-2 border rounded-md  border-gray-600">
        <div className="gap-2 md:gap-3 flex flex-col">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 16 16"
              width={16}
              height={16}
              className="mr-2"
            >
              <path fill="#fff" d="M0 0h16v16H0z" />
              <path
                stroke="#3F3F46"
                strokeLinejoin="round"
                strokeWidth="1.5"
                d="M13.5 13.5h-11V9h3V5.5h4V2h4v11.5z"
              />
            </svg>
            <h1 className="font-bold text-xs">Langkah 2</h1>
          </div>
          <span className="font-bold text-base">
            Belajar Dasar Pemrograman JavaScript
          </span>
          <div className="flex gap-9">
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="text-green-400"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M8 14.667A6.667 6.667 0 1 0 8 1.334a6.667 6.667 0 0 0 0 13.333Zm.667-10a.667.667 0 0 0-1.333 0v3.334c0 .176.07.346.195.471l2.334 2.333a.667.667 0 0 0 .942-.942L8.667 7.725V4.667Z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-semibold text-xs">45 jam</p>
            </div>
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
                className="course-card__icon text-yellow-400"
              >
                <path
                  d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                  fill=""
                />
              </svg>
              <p className="font-semibold text-xs">4,86</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-400"
                width={16}
                height={16}
              >
                <rect
                  width="2.667"
                  height="4.667"
                  x="2.667"
                  y={8}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="6.667"
                  x="6.667"
                  y={6}
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="9.333"
                  x="10.667"
                  y="3.333"
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
              </svg>
              <p className="font-semibold text-xs">Dasar</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                className="course-card__icon text-gray-500"
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1.33334 2.66666C1.33334 1.93028 1.9303 1.33333 2.66668 1.33333H10C10.7364 1.33333 11.3333 1.93028 11.3333 2.66666V3.33333H13.3333C14.0697 3.33333 14.6667 3.93028 14.6667 4.66666V13.3333C14.6667 14.0697 14.0697 14.6667 13.3333 14.6667H2.66668C1.9303 14.6667 1.33334 14.0697 1.33334 13.3333V2.66666ZM10 2.66666V3.33333H2.66668V2.66666H10ZM2.66668 13.3333V4.66666H4.66668V13.3333H2.66668ZM6.00001 13.3333H13.3333V4.66666H6.00001V13.3333Z"
                ></path>
              </svg>
              <p className="font-normal text-xs">71,modul</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-500"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M5 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM4 5a1 1 0 112 0 1 1 0 01-2 0zM11 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM10 5a1 1 0 112 0 1 1 0 01-2 0zM8 9.558a3.667 3.667 0 00-6.667 2.109V14c0 .368.299.667.667.667h12a.667.667 0 00.667-.667v-2.333A3.667 3.667 0 008 9.558zm-.667 3.775v-1.668a2.333 2.333 0 00-4.666.002v1.666h4.666zm1.334-1.668v1.668h4.666v-1.666a2.333 2.333 0 00-4.666-.002z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-normal text-xs">5.313 Siswa terdaftar</p>
            </div>
          </div>
        </div>
      </div>
      <div className="flex mx-auto md:gap-10  gap-1 flex-col">
        <div className="h-10 md:h-32 bg-dark  w-[2px] md:mt-0 mt-2  mx-auto flex" />
        <span className="font-bold flex justify-center">2</span>
      </div>
      <span className="flex text-center md:mt-0 mt-10 flex-col gap-4">
        <h1 className="font-bold text-xl">Kuasai JavaScript</h1>
        <div className="flex justify-center">
          <p className="font-medium md:w-[50%]">
            Kuasai JavaScript sebagai bahasa pemrograman populer, mulai dari
            fundamental, sintaks dan fitur ES6, hingga konsep automation test
            menggunakan framework ternama.
          </p>
        </div>
      </span>
    </div>
    <div className="flex-col md:flex mt-8 justify-between w-full items-center">
      <div className=" px-6 md:w-[370px] sm:h-[164px] flex items-center hover:shadow-lg mx-auto h-auto py-2 border rounded-md border-gray-600 ">
        <div className="gap-2 md:gap-4 flex flex-col">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 16 16"
              width={16}
              height={16}
              className="mr-2"
            >
              <path fill="#fff" d="M0 0h16v16H0z" />
              <path
                stroke="#3F3F46"
                strokeLinejoin="round"
                strokeWidth="1.5"
                d="M13.5 13.5h-11V9h3V5.5h4V2h4v11.5z"
              />
            </svg>
            <h1 className="font-bold text-xs">Langkah 3</h1>
          </div>
          <span className="font-bold text-base">
            Belajar Membuat Front-End Web untuk Pemula
          </span>
          <div className="flex gap-9">
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="text-green-400"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M8 14.667A6.667 6.667 0 1 0 8 1.334a6.667 6.667 0 0 0 0 13.333Zm.667-10a.667.667 0 0 0-1.333 0v3.334c0 .176.07.346.195.471l2.334 2.333a.667.667 0 0 0 .942-.942L8.667 7.725V4.667Z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-semibold text-xs">45 jam</p>
            </div>
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
                className="course-card__icon text-yellow-400"
              >
                <path
                  d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                  fill=""
                />
              </svg>
              <p className="font-semibold text-xs">4,87</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-400"
                width={16}
                height={16}
              >
                <rect
                  width="2.667"
                  height="4.667"
                  x="2.667"
                  y={8}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="6.667"
                  x="6.667"
                  y={6}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="9.333"
                  x="10.667"
                  y="3.333"
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
              </svg>
              <p className="font-semibold text-xs">Pemula</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                className="course-card__icon text-gray-500"
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1.33334 2.66666C1.33334 1.93028 1.9303 1.33333 2.66668 1.33333H10C10.7364 1.33333 11.3333 1.93028 11.3333 2.66666V3.33333H13.3333C14.0697 3.33333 14.6667 3.93028 14.6667 4.66666V13.3333C14.6667 14.0697 14.0697 14.6667 13.3333 14.6667H2.66668C1.9303 14.6667 1.33334 14.0697 1.33334 13.3333V2.66666ZM10 2.66666V3.33333H2.66668V2.66666H10ZM2.66668 13.3333V4.66666H4.66668V13.3333H2.66668ZM6.00001 13.3333H13.3333V4.66666H6.00001V13.3333Z"
                ></path>
              </svg>
              <p className="font-normal text-xs">67,modul</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-500"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M5 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM4 5a1 1 0 112 0 1 1 0 01-2 0zM11 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM10 5a1 1 0 112 0 1 1 0 01-2 0zM8 9.558a3.667 3.667 0 00-6.667 2.109V14c0 .368.299.667.667.667h12a.667.667 0 00.667-.667v-2.333A3.667 3.667 0 008 9.558zm-.667 3.775v-1.668a2.333 2.333 0 00-4.666.002v1.666h4.666zm1.334-1.668v1.668h4.666v-1.666a2.333 2.333 0 00-4.666-.002z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-normal text-xs">48.142 Siswa terdaftar</p>
            </div>
          </div>
        </div>
      </div>
      <div className="flex mx-auto  md:gap-10  gap-1 flex-col">
        <div className="h-10 md:h-32 bg-dark w-[2px] md:mt-0 mt-2  mx-auto flex" />
        <span className="font-bold flex justify-center ">3</span>
      </div>
      <span className="flex text-center  md:mt-0 mt-10 flex-col gap-4">
        <h1 className="font-bold text-xl">Buat Webmu Menjadi Interaktif</h1>
        <div className="flex justify-center">
          <p className="font-medium md:w-[70%]">
            Seorang Front-End Web Developer harus mahir menggunakan JavaScript
            agar mampu membuat website yang interaktif. Di langkah ini, Anda
            akan belajar banyak tentang JavaScript untuk membuat websitemu
            menarik.
          </p>
        </div>
      </span>
    </div>
    <div className="flex-col md:flex mt-8 justify-between w-full items-center">
      <div className=" px-6 md:w-[370px] sm:h-[164px] flex items-center hover:shadow-lg mx-auto h-auto py-2 border rounded-md border-gray-600 ">
        <div className="gap-3 flex flex-col">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 16 16"
              width={16}
              height={16}
              className="mr-2"
            >
              <path fill="#fff" d="M0 0h16v16H0z" />
              <path
                stroke="#3F3F46"
                strokeLinejoin="round"
                strokeWidth="1.5"
                d="M13.5 13.5h-11V9h3V5.5h4V2h4v11.5z"
              />
            </svg>
            <h1 className="font-bold text-xs">Langkah 4</h1>
          </div>
          <span className="font-bold text-base">
            Belajar Fundamental Front-End Web Development
          </span>
          <div className="flex gap-9">
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="text-green-400"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M8 14.667A6.667 6.667 0 1 0 8 1.334a6.667 6.667 0 0 0 0 13.333Zm.667-10a.667.667 0 0 0-1.333 0v3.334c0 .176.07.346.195.471l2.334 2.333a.667.667 0 0 0 .942-.942L8.667 7.725V4.667Z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-semibold text-xs">70 jam</p>
            </div>
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
                className="course-card__icon text-yellow-400"
              >
                <path
                  d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                  fill=""
                />
              </svg>
              <p className="font-semibold text-xs">4,87</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-400"
                width={16}
                height={16}
              >
                <rect
                  width="2.667"
                  height="4.667"
                  x="2.667"
                  y={8}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="6.667"
                  x="6.667"
                  y={6}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="9.333"
                  x="10.667"
                  y="3.333"
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
              </svg>
              <p className="font-semibold text-xs">Pemula</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                className="course-card__icon text-gray-500"
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1.33334 2.66666C1.33334 1.93028 1.9303 1.33333 2.66668 1.33333H10C10.7364 1.33333 11.3333 1.93028 11.3333 2.66666V3.33333H13.3333C14.0697 3.33333 14.6667 3.93028 14.6667 4.66666V13.3333C14.6667 14.0697 14.0697 14.6667 13.3333 14.6667H2.66668C1.9303 14.6667 1.33334 14.0697 1.33334 13.3333V2.66666ZM10 2.66666V3.33333H2.66668V2.66666H10ZM2.66668 13.3333V4.66666H4.66668V13.3333H2.66668ZM6.00001 13.3333H13.3333V4.66666H6.00001V13.3333Z"
                ></path>
              </svg>
              <p className="font-normal text-xs">143,modul</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-500"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M5 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM4 5a1 1 0 112 0 1 1 0 01-2 0zM11 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM10 5a1 1 0 112 0 1 1 0 01-2 0zM8 9.558a3.667 3.667 0 00-6.667 2.109V14c0 .368.299.667.667.667h12a.667.667 0 00.667-.667v-2.333A3.667 3.667 0 008 9.558zm-.667 3.775v-1.668a2.333 2.333 0 00-4.666.002v1.666h4.666zm1.334-1.668v1.668h4.666v-1.666a2.333 2.333 0 00-4.666-.002z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-normal text-xs">25.563 Siswa terdaftar</p>
            </div>
          </div>
        </div>
      </div>
      <div className="flex mx-auto md:gap-10  gap-1 flex-col">
        <div className="h-10 md:h-32 bg-dark w-[2px] md:mt-0 mt-2  mx-auto flex" />
        <span className="font-bold flex justify-center">4</span>
      </div>
      <span className="flex text-center md:mt-0 mt-10 flex-col gap-4">
        <h1 className="font-bold text-xl">
          Pelajari Hal Esensial di Bidang Front-End Web Development
        </h1>
        <div className="flex justify-center">
          <p className="font-medium md:w-[70%]">
            Agar pengalaman mengembangkan website lebih seru, gunakanlah webpack
            sebagai module bundler. Di sini, Anda juga akan belajar cara
            menampilkan data dari server sehingga aplikasi dapat menampilkan
            data yang dinamis.
          </p>
        </div>
      </span>
    </div>
    <div className="flex-col md:flex mt-8 justify-between w-full items-center">
      <div className=" px-6 md:w-[370px] sm:h-[164px] flex items-center hover:shadow-lg mx-auto h-auto py-2 border rounded-md border-gray-600 ">
        <div className="gap-2 md:gap-4 flex flex-col">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 16 16"
              width={16}
              height={16}
              className="mr-2"
            >
              <path fill="#fff" d="M0 0h16v16H0z" />
              <path
                stroke="#3F3F46"
                strokeLinejoin="round"
                strokeWidth="1.5"
                d="M13.5 13.5h-11V9h3V5.5h4V2h4v11.5z"
              />
            </svg>
            <h1 className="font-bold text-xs">Langkah 5</h1>
          </div>
          <span className="font-bold text-base">
            Belajar Tools Front-End Web Intermediate
          </span>
          <div className="flex gap-9">
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="text-green-400"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M8 14.667A6.667 6.667 0 1 0 8 1.334a6.667 6.667 0 0 0 0 13.333Zm.667-10a.667.667 0 0 0-1.333 0v3.334c0 .176.07.346.195.471l2.334 2.333a.667.667 0 0 0 .942-.942L8.667 7.725V4.667Z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-semibold text-xs">65 jam</p>
            </div>
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
                className="course-card__icon text-yellow-400"
              >
                <path
                  d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                  fill=""
                />
              </svg>
              <p className="font-semibold text-xs">4,80</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-400"
                width={16}
                height={16}
              >
                <rect
                  width="2.667"
                  height="4.667"
                  x="2.667"
                  y={8}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="6.667"
                  x="6.667"
                  y={6}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="9.333"
                  x="10.667"
                  y="3.333"
                  rx={1}
                  className="text-gray-200"
                  fill="currentColor"
                />
              </svg>
              <p className="font-semibold text-xs">Mahir</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                className="course-card__icon text-gray-500"
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1.33334 2.66666C1.33334 1.93028 1.9303 1.33333 2.66668 1.33333H10C10.7364 1.33333 11.3333 1.93028 11.3333 2.66666V3.33333H13.3333C14.0697 3.33333 14.6667 3.93028 14.6667 4.66666V13.3333C14.6667 14.0697 14.0697 14.6667 13.3333 14.6667H2.66668C1.9303 14.6667 1.33334 14.0697 1.33334 13.3333V2.66666ZM10 2.66666V3.33333H2.66668V2.66666H10ZM2.66668 13.3333V4.66666H4.66668V13.3333H2.66668ZM6.00001 13.3333H13.3333V4.66666H6.00001V13.3333Z"
                ></path>
              </svg>
              <p className="font-normal text-xs">131,modul</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-500"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M5 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM4 5a1 1 0 112 0 1 1 0 01-2 0zM11 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM10 5a1 1 0 112 0 1 1 0 01-2 0zM8 9.558a3.667 3.667 0 00-6.667 2.109V14c0 .368.299.667.667.667h12a.667.667 0 00.667-.667v-2.333A3.667 3.667 0 008 9.558zm-.667 3.775v-1.668a2.333 2.333 0 00-4.666.002v1.666h4.666zm1.334-1.668v1.668h4.666v-1.666a2.333 2.333 0 00-4.666-.002z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-normal text-xs">849 Siswa terdaftar</p>
            </div>
          </div>
        </div>
      </div>
      <div className="flex mx-auto md:gap-10  gap-1 flex-col">
        <div className="h-10 md:h-32 bg-dark  w-[2px] md:mt-0 mt-2  mx-auto flex" />
        <span className="font-bold flex justify-center ">5</span>
      </div>
      <span className="flex text-center  md:mt-0 mt-10 flex-col gap-4 ">
        <h1 className="font-bold text-xl">
          Implementasi Tools Berstandar Industri di Sini
        </h1>
        <div className="flex justify-center">
          <p className="font-medium md:w-[60%]">
            Persiapkan skill-mu sebagai langkah awal untuk memasuki dunia
            industri dengan mempelajari tools untuk meningkatkan kualitas
            aplikasi web kamu. Di langkah ini, kamu akan mempelajari tools yang
            banyak digunakan di industri.
          </p>
        </div>
      </span>
    </div>
    <div className="flex-col md:flex mt-8  w-full items-center">
      <div className="px-6 md:w-[370px] sm:h-[164px] flex items-center hover:shadow-lg mx-auto h-auto py-2  border rounded-md border-gray-600">
        <div className="gap-4 flex flex-col">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 16 16"
              width={16}
              height={16}
              className="mr-2"
            >
              <path fill="#fff" d="M0 0h16v16H0z" />
              <path
                stroke="#3F3F46"
                strokeLinejoin="round"
                strokeWidth="1.5"
                d="M13.5 13.5h-11V9h3V5.5h4V2h4v11.5z"
              />
            </svg>
            <h1 className="font-bold text-xs">Langkah 6</h1>
          </div>
          <span className="font-bold text-base">
            Menjadi Front-End Web Developer Expert
          </span>
          <div className="flex gap-9">
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="text-green-400"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M8 14.667A6.667 6.667 0 1 0 8 1.334a6.667 6.667 0 0 0 0 13.333Zm.667-10a.667.667 0 0 0-1.333 0v3.334c0 .176.07.346.195.471l2.334 2.333a.667.667 0 0 0 .942-.942L8.667 7.725V4.667Z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-semibold text-xs">100 jam</p>
            </div>
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
                className="course-card__icon text-yellow-400"
              >
                <path
                  d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                  fill=""
                />
              </svg>
              <p className="font-semibold text-xs">4,88</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-400"
                width={16}
                height={16}
              >
                <rect
                  width="2.667"
                  height="4.667"
                  x="2.667"
                  y={8}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="6.667"
                  x="6.667"
                  y={6}
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
                <rect
                  width="2.667"
                  height="9.333"
                  x="10.667"
                  y="3.333"
                  rx={1}
                  className="text-blue-400"
                  fill="currentColor"
                />
              </svg>
              <p className="font-semibold text-xs">Profesional</p>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="flex gap-3">
              <svg
                width={16}
                height={16}
                className="course-card__icon text-gray-500"
                viewBox="0 0 16 16"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1.33334 2.66666C1.33334 1.93028 1.9303 1.33333 2.66668 1.33333H10C10.7364 1.33333 11.3333 1.93028 11.3333 2.66666V3.33333H13.3333C14.0697 3.33333 14.6667 3.93028 14.6667 4.66666V13.3333C14.6667 14.0697 14.0697 14.6667 13.3333 14.6667H2.66668C1.9303 14.6667 1.33334 14.0697 1.33334 13.3333V2.66666ZM10 2.66666V3.33333H2.66668V2.66666H10ZM2.66668 13.3333V4.66666H4.66668V13.3333H2.66668ZM6.00001 13.3333H13.3333V4.66666H6.00001V13.3333Z"
                ></path>
              </svg>
              <p className="font-normal text-xs">191,modul</p>
            </div>
            <div className="flex gap-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                className="course-card__icon text-gray-500"
                width={16}
                height={16}
              >
                <path
                  fillRule="evenodd"
                  d="M5 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM4 5a1 1 0 112 0 1 1 0 01-2 0zM11 2.667a2.333 2.333 0 100 4.666 2.333 2.333 0 000-4.666zM10 5a1 1 0 112 0 1 1 0 01-2 0zM8 9.558a3.667 3.667 0 00-6.667 2.109V14c0 .368.299.667.667.667h12a.667.667 0 00.667-.667v-2.333A3.667 3.667 0 008 9.558zm-.667 3.775v-1.668a2.333 2.333 0 00-4.666.002v1.666h4.666zm1.334-1.668v1.668h4.666v-1.666a2.333 2.333 0 00-4.666-.002z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="font-normal text-xs">6.134 Siswa terdaftar</p>
            </div>
          </div>
        </div>
      </div>
      <div className="flex mx-auto  md:gap-10  gap-1 flex-col">
        <div className="h-10 md:h-32 bg-dark  w-[2px] md:mt-0 mt-2  mx-auto flex" />
        <span className="font-bold flex justify-center ">6</span>
      </div>
      <span className="flex text-center  md:mt-0 mt-10 flex-col gap-4">
        <h1 className="font-bold text-xl">
          Jadilah Expert di Bidang Front-End
        </h1>
        <div className="flex justify-center">
          <p className="font-medium md:w-[70%]">
            Menjadi seorang expert adalah impian setiap developer. Untuk itu,
            Anda perlu mempelajari prinsip dan materi expert seperti testing dan
            performance di sini. Di langkah ini juga, Anda diarahkan untuk
            membuat web aplikasi yang modern.
          </p>
        </div>
      </span>
    </div>
  </main>
  <div className="mt-32  flex flex-col justify-center gap-10 items-center">
    <span className="text-center font-bold text-3xl flex justify-center items-center">
      Jadilah Front-End Web Developer seperti mereka
    </span>
    <div className="flex hover:gap-3 transition duration-300">
      <span className="border-[3px] border-dark rounded-full relative z-10 m-mauto inline-block  hover:m-auto  transition ease-out duration-200">
        <img
          src="https://d17ivq9b7rppb3.cloudfront.net/small/avatar/20191008120045bb836b265c2f09ed9f174c058ac7c8b1.jpg"
          alt=""
          className="w-[42px] h-[42px] rounded-full"
        />
      </span>
      <span className="border-[3px] border-dark rounded-full relative z-10 m-mauto  hover:m-auto inline-block transition ease-out duration-200">
        <img
          src="https://d17ivq9b7rppb3.cloudfront.net/small/avatar/20200327173842f1c33af847e58d5ccf549adfe6fff62c.png"
          alt=""
          className="w-[42px] h-[42px] rounded-full"
        />
      </span>
    </div>
    <button className=" w-[200px] flex items-center justify-center py-3  text-white font-semibold rounded-3xl scroll-smooth bg-dark">
      <a href="#" onclick="goBuy()">
        Beli Sekarang
      </a>
    </button>
  </div>
  <main className="mt-32 w-full h-auto py-9 px-10 md:px-32 bg-gray-50  ">
    <div className="flex flex-col gap-7 items-center justify-center">
      <span className="font-bold text-3xl text-center flex justify-center">
        Apa kata lulusan lopercourse
      </span>
      <div className="flex gap-10 flex-wrap justify-center items-center">
        <div className="border md:w-[500px] rounded-md w-full h-[300px] border-gray-500">
          <div className="flex top-0 left-0 ml-3 mt-4 mb-3">
            <img
              src="https://dicoding-web-img.sgp1.cdn.digitaloceanspaces.com/small/avatar/dos:abad150dc49346f9bf1a883377bf5e2620221216013257.png"
              className="rounded-full w-10 h-10 border-2 border-dark p-[1px] "
            />
            <div className="ml-3">
              <div className="text-sm text-gray-700 ]">
                Geraldo Sepdwijaya
              </div>
              <div className="flex flex-col gap-1">
                <div className="text-xs flex text-gray-400 mt-1">
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="mr-1 text-gray-700"
                  >
                    <path
                      d="M7.33325 5.33203H4.66659V3.9987H7.33325V5.33203Z"
                      fill="#3F3F46"
                    ></path>
                    <path
                      d="M4.66659 7.9987H7.33325V6.66537H4.66659V7.9987Z"
                      fill="#3F3F46"
                    />
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M1.99992 1.9987C1.99992 1.63051 2.2984 1.33203 2.66659 1.33203H9.33325C9.70144 1.33203 9.99992 1.63051 9.99992 1.9987V11.9987H12.6666V6.66537H11.3333V5.33203H13.3333C13.7014 5.33203 13.9999 5.63051 13.9999 5.9987V11.9987H14.6666V13.332H1.33325V11.9987H1.99992V1.9987ZM3.33325 11.9987H8.66659V2.66536H3.33325V11.9987Z"
                      fill="#3F3F46"
                    />
                  </svg>
                  PT. Maybank Indonesia
                </div>
                <div className="text-xs flex text-gray-400 mt-1 ">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    className="mr-1 text-gray-700 "
                    width={16}
                    height={16}
                  >
                    <path
                      fillRule="evenodd"
                      d="M11.514 2.126a1 1 0 0 1 .972 0l9 5a1 1 0 0 1 0 1.748L20 9.7V16a1 1 0 0 1-.725.962l-2.275.65V21a1 1 0 1 1-2 0v-2.817l-2.725.779a1 1 0 0 1-.55 0l-7-2A1 1 0 0 1 4 16V9.7l-1.486-.826a1 1 0 0 1 0-1.748l9-5ZM17 15.53l1-.285V10.81l-1 .555v4.165Zm-2-3.054v3.626l-3 .857-6-1.714V10.81l5.514 3.063a1 1 0 0 0 .972 0L15 12.477Zm1.03-2.86-2.323-2.324a1 1 0 1 0-1.414 1.414l1.92 1.92L12 11.856 5.06 8 12 4.144 18.94 8l-2.91 1.617Z"
                      clipRule="evenodd"
                    />
                  </svg>
                  Menjadi Front-End Web Developer Expert
                </div>
                <div className="text-gray-400 flex">
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                </div>
              </div>
            </div>
          </div>
          <div className="text-gray-700 mx-3 font-medium ">
            Kelas ini materinya sangat lengkap dan mudah dipahami, reviewernya
            juga selalu memberikan saran agar dapat menuliskan kode dengan baik
            saat mengerjakan submission.
          </div>
        </div>
        <div className="border md:w-[500px] rounded-md w-full h-auto pb-4 sm:h-[300px]  border-gray-500">
          <div className="flex top-0 left-0 ml-3 mt-4 mb-3">
            <img
              src="https://d17ivq9b7rppb3.cloudfront.net/small/avatar/2021052711584016c588284bf009c1924299e4c6e12b4f.png"
              className="w-10 h-10 rounded-full p-[1px] border-2 border-dark "
            />
            <div className="ml-3">
              <div className="text-sm text-gray-700 ">
                Lukman Bahar Agung
              </div>
              <div className="flex flex-col gap-1">
                <div className="text-xs text-gray-400 mt-1  flex">
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="mr-1 text-gray-700"
                  >
                    <path
                      d="M7.33325 5.33203H4.66659V3.9987H7.33325V5.33203Z"
                      fill="#3F3F46"
                    ></path>
                    <path
                      d="M4.66659 7.9987H7.33325V6.66537H4.66659V7.9987Z"
                      fill="#3F3F46"
                    />
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M1.99992 1.9987C1.99992 1.63051 2.2984 1.33203 2.66659 1.33203H9.33325C9.70144 1.33203 9.99992 1.63051 9.99992 1.9987V11.9987H12.6666V6.66537H11.3333V5.33203H13.3333C13.7014 5.33203 13.9999 5.63051 13.9999 5.9987V11.9987H14.6666V13.332H1.33325V11.9987H1.99992V1.9987ZM3.33325 11.9987H8.66659V2.66536H3.33325V11.9987Z"
                      fill="#3F3F46"
                    />
                  </svg>
                  Alodokter
                </div>
                <div className="text-xs text-gray-400 mt-1 flex ">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    className="mr-1 text-gray-700 "
                    width={16}
                    height={16}
                  >
                    <path
                      fillRule="evenodd"
                      d="M11.514 2.126a1 1 0 0 1 .972 0l9 5a1 1 0 0 1 0 1.748L20 9.7V16a1 1 0 0 1-.725.962l-2.275.65V21a1 1 0 1 1-2 0v-2.817l-2.725.779a1 1 0 0 1-.55 0l-7-2A1 1 0 0 1 4 16V9.7l-1.486-.826a1 1 0 0 1 0-1.748l9-5ZM17 15.53l1-.285V10.81l-1 .555v4.165Zm-2-3.054v3.626l-3 .857-6-1.714V10.81l5.514 3.063a1 1 0 0 0 .972 0L15 12.477Zm1.03-2.86-2.323-2.324a1 1 0 1 0-1.414 1.414l1.92 1.92L12 11.856 5.06 8 12 4.144 18.94 8l-2.91 1.617Z"
                      clipRule="evenodd"
                    />
                  </svg>
                  Menjadi Front-End Web Developer Expert
                </div>
                <div className="text-gray-400 flex">
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                  <svg
                    width={16}
                    height={16}
                    viewBox="0 0 16 16"
                    fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg"
                    className="text-yellow-400"
                  >
                    <path
                      d="M8 0L10.2899 4.84829L15.6085 5.52786L11.7051 9.20385L12.7023 14.4721L8 11.8957L3.29772 14.4721L4.29494 9.20385L0.391548 5.52786L5.71015 4.84829L8 0Z"
                      fill=""
                    />
                  </svg>
                </div>
              </div>
            </div>
          </div>
          <div className="text-gray-700 mx-3 font-medium ">
            Kelas ini menggiring pola pikir kita menjadi seorang expert, materi
            kelasnya padat. setelah lulus, bisa membuat karya web dengan
            performance, accessibility, best practice dan PWA dengan audit
            score.
          </div>
        </div>
      </div>
    </div>
  </main>

    <Footer/>
</>
  )}